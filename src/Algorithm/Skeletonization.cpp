...
double Skeletonization::getMaxAngleOfTwoPair(vector<Point3f>& dir0, vector<Point3f>& dir1)
{
 ...
 Branch Skeletonization::mergeTowBranches(Branch& branch0, Branch& branch1, CONNECT_TYPE C_Type)
 {
   ...
   case H0_H1: {
     ...
     auto r0c = reverseOneCurve(c0);
     new_curve = combineTwoCurvesInOrder(r0c, c1);
   ...
   case H0_T1: {
     ...
     auto r0c = reverseOneCurve(c0);
     auto r1c = reverseOneCurve(c1);
     new_curve = combineTwoCurvesInOrder(r0c, r1c);
     new_branch.back_up_head = branch0.back_up_tail;
   ...
   case T0_T1: {
     ...
     auto r1c = reverseOneCurve(c1);
     new_curve = combineTwoCurvesInOrder(c0, r1c);
     new_branch.back_up_head = branch0.back_up_head;
     new_branch.back_up_tail = branch1.back_up_head;
...
void Skeletonization::dealWithVirtualsForTail(Branch& branch)
{
  ...
  if (branch.isHeadVirtual())
  {
    auto bhead = branch.getHead();
    auto btail = branch.getTail();
    double head_tail_dist2 = GlobalFun::computeEulerDistSquare(bhead, btail);
...
void Skeletonization::reconnectSkeleton()
{
  ...
  while(1)
  {
    ...
    for (int i = 0; i < branches.size(); i++)
    {
      ...
      auto head_P = branch.getHead();
      auto tail_P = branch.getTail();
  ...
  while(1)
  {
    ...
    for (int i = 0; i < branches.size(); i++)
    {
      ...
      auto head0_P = branch0.getHead();
      auto tail0_P = branch0.getTail();
      ...
      for (int j = 0; j < branches.size(); j++)
      {
        ...
        auto head1_P = branch1.getHead();
        auto tail1_P = branch1.getTail();
